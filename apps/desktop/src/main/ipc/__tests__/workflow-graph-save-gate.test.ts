 ...
 *    数据）必须通过形状+拓扑双闸门；
 * ② skill 产物 JSON 回放：spark-workflow-generator（PR #187 已合入）的 templates/
 *    与 demos/ 逐个 graph 正向回放；demos 里刻意做坏的负向样本单独断言必须被拒；
...
/**
 * skill demos 里刻意做坏的负向样本（文件自述「校验器负向测试用，勿导入」，埋了未知
 * kind 等 5 类错误）。它不参与正向回放，改由下方专门用例断言双闸门必须拒绝。
 */
const NEGATIVE_FIXTURES = new Set(['test-bad-sample.json'])
...
describe('workflow save gate — integration replay', () => {
  ...
  describe.skipIf(!existsSync(SKILL_DIR))('official skill artifact replay', () => {
    ...
    it('every skill demo passes both gates, including the wrapped import format', () => {
      const files = listJsonFiles(join(SKILL_DIR, 'demos')).filter(
        (file) => !NEGATIVE_FIXTURES.has(basename(file)),
      )
    ...
    it('the intentional bad sample stays rejected by both gates', () => {
      for (const name of NEGATIVE_FIXTURES) {
        const file = join(SKILL_DIR, 'demos', name)
        if (!existsSync(file)) continue
        for (const graph of extractGraphs(readFileSync(file, 'utf8'))) {
          expect(() => assertWorkflowGraphSchema(graph), file).toThrow()
          expect(() => assertWorkflowGraphValid(graph), file).toThrow()
        }
      }
    })
  })
